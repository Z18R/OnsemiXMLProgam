...
Public Class main
    ...
    Dim backup As String = "D:\onsemi\xml\backup\On Semi Xml backup\" & folder & "\"
    Dim backup2 As String = "F:\backupinF\" & folder & "\"
    Dim sftp As String = "D:\onsemi\xml\de_xml\" ' TEST
...
attemp:
            ...
            End If
            If Not Directory.Exists(backup2) Then
...
attemp:
            ...
            For Each files As String In fileList
                ...
                System.IO.File.Copy(files, backup2 & fileName, True)
...
skip:
    ...
    Public Shared Function dtReport() As DataTable
        ...
        'Dim strSQL As String = "usp_TRN_ONSemi_xml '2023-02-18 13:00:01.000','2023-02-18 15:00:01.000'"
    ...
    Private Sub SendEmail(ByVal strSubject As String, ByVal strMessage As String, ByVal xmlfile As String, ByVal trgfile As String)
        ...
        Dim Sqlemail As String = "usp_SPT_AutoEmail_GetRecipients 141"
    ...
    Private Sub SendEmailerror(ByVal strSubject As String, ByVal strMessage As String)
        ...
        'SmtpMail.UseDefaultCredentials = True
        'SmtpMail.Credentials = New System.Net.NetworkCredential("administrator", "trator#$0809")
        Dim Sql As String = "SELECT * FROM  ATEC_EmailServer WHERE ID = 2"
        ds = FillDataset(Sql, ConnectionString)
        Dim Username, Password As String
        Dim SmtpMail As New SmtpClient
        SmtpMail.Host = ds.Tables(0).Rows(0).Item("Host").ToString() '"Atec-mail"
